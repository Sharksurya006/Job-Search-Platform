		 ...
		 <Link to="/jobs"><div className="w-[1/3] border-1 border-gray-400 p-6 flex flex-col gap-5 rounded-lg hover:shadow-[0_0_5px_1px_green]">
			...
			<div className="flex items-center justify-around">
				...
				<div className="flex gap-2 items-center">
					<p><IoMdTime /></p>
					<div><p>{job.applied ? "Applied" : "posted"} {job.posted}</p></div>
				</div>
			</div>
			<div>
			{job.offered && <Divider className="my-6" size="xs" color="black"/>}
			{job.offered && <div className="flex justify-between my-2 mx-4">
				<button className="px-6 py-3 bg-black text-white hover:bg-green-500 rounded-lg hover:text-black">Accept</button>
				<button className="px-6 py-2 bg-black text-white hover:bg-green-500 rounded-lg  hover:text-black">Reject</button>
				</div>}
		</div>
		<div>
			{job.interviewing && <Divider className="my-6" size="xs" color="black"/>}
			{
				job.interviewing && <div className="flex gap-2.5">
